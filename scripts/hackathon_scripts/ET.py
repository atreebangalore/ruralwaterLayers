...
def get_rainfall(agg_col, pattern, unit, reducer, band_name, data):
...
def save_CSV(df, parameter, timestep, village_name, data):
...
yr_df = get_rainfall(hyd_yr_col, '%Y', 'kg/m2', 'sum', 'ET', 'MODIS')
filepath = save_CSV(yr_df, 'TotalET', 'hydYear', village_name, 'MODIS')
print(f'completed: {filepath}')

hyd_yr_col = ee.ImageCollection(ee.List(year_list).map(lambda year: yearly_sum(year, 'mean', 'MODIS', 'ET')))
yr_df = get_rainfall(hyd_yr_col, '%Y', 'kg/m2/8day', 'mean', 'ET', 'MODIS')
filepath = save_CSV(yr_df, 'MeanET', 'hydYear', village_name, 'MODIS')
print(f'completed: {filepath}')

hyd_yr_col = ee.ImageCollection(ee.List(year_list).map(lambda year: yearly_sum(year, 'median', 'MODIS', 'ET')))
yr_df = get_rainfall(hyd_yr_col, '%Y', 'kg/m2/8day', 'median', 'ET', 'MODIS')
filepath = save_CSV(yr_df, 'MedianET', 'hydYear', village_name, 'MODIS')
print(f'completed: {filepath}')

hyd_yr_col = ee.ImageCollection(ee.List(year_list).map(lambda year: yearly_sum(year, 'max', 'MODIS', 'ET')))
yr_df = get_rainfall(hyd_yr_col, '%Y', 'kg/m2/8day', 'max', 'ET', 'MODIS')
filepath = save_CSV(yr_df, 'MaxET', 'hydYear', village_name, 'MODIS')
print(f'completed: {filepath}')

hyd_yr_col = ee.ImageCollection(ee.List(year_list).map(lambda year: yearly_sum(year, 'min', 'MODIS', 'ET')))
yr_df = get_rainfall(hyd_yr_col, '%Y', 'kg/m2/8day', 'min', 'ET', 'MODIS')
filepath = save_CSV(yr_df, 'MinET', 'hydYear', village_name, 'MODIS')
print(f'completed: {filepath}')

hyd_yr_col = ee.ImageCollection(ee.List(year_list).map(lambda year: yearly_sum(year, 'sum', 'SSEBop', 'b1')))
yr_df = get_rainfall(hyd_yr_col, '%Y', 'mm', 'sum', 'b1', 'SSEBop')
filepath = save_CSV(yr_df, 'TotalET', 'hydYear', village_name, 'SSEBop')
print(f'completed: {filepath}')

hyd_yr_col = ee.ImageCollection(ee.List(year_list).map(lambda year: yearly_sum(year, 'mean', 'SSEBop', 'b1')))
yr_df = get_rainfall(hyd_yr_col, '%Y', 'mm/month', 'mean', 'b1', 'SSEBop')
filepath = save_CSV(yr_df, 'MeanET', 'hydYear', village_name, 'SSEBop')
print(f'completed: {filepath}')

hyd_yr_col = ee.ImageCollection(ee.List(year_list).map(lambda year: yearly_sum(year, 'median', 'SSEBop', 'b1')))
yr_df = get_rainfall(hyd_yr_col, '%Y', 'mm/month', 'median', 'b1', 'SSEBop')
filepath = save_CSV(yr_df, 'MedianET', 'hydYear', village_name, 'SSEBop')
print(f'completed: {filepath}')

hyd_yr_col = ee.ImageCollection(ee.List(year_list).map(lambda year: yearly_sum(year, 'max', 'SSEBop', 'b1')))
yr_df = get_rainfall(hyd_yr_col, '%Y', 'mm/month', 'max', 'b1', 'SSEBop')
filepath = save_CSV(yr_df, 'MaxET', 'hydYear', village_name, 'SSEBop')
print(f'completed: {filepath}')

hyd_yr_col = ee.ImageCollection(ee.List(year_list).map(lambda year: yearly_sum(year, 'min', 'SSEBop', 'b1')))
yr_df = get_rainfall(hyd_yr_col, '%Y', 'mm/month', 'min', 'b1', 'SSEBop')
filepath = save_CSV(yr_df, 'MinET', 'hydYear', village_name, 'SSEBop')
print(f'completed: {filepath}')

hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'sum', 'MODIS', 'ET')))
mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'kg/m2', 'sum', 'ET', 'MODIS')
filepath = save_CSV(mn_df, 'TotalET', 'monthly', village_name, 'MODIS')
print(f'completed: {filepath}')

hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'mean', 'MODIS', 'ET')))
mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'kg/m2/8day', 'mean', 'ET', 'MODIS')
filepath = save_CSV(mn_df, 'MeanET', 'monthly', village_name, 'MODIS')
print(f'completed: {filepath}')

hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'median', 'MODIS', 'ET')))
mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'kg/m2/8day', 'median', 'ET', 'MODIS')
filepath = save_CSV(mn_df, 'MedianET', 'monthly', village_name, 'MODIS')
print(f'completed: {filepath}')

hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'max', 'MODIS', 'ET')))
mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'kg/m2/8day', 'max', 'ET', 'MODIS')
filepath = save_CSV(mn_df, 'MaxET', 'monthly', village_name, 'MODIS')
print(f'completed: {filepath}')

hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'min', 'MODIS', 'ET')))
mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'kg/m2/8day', 'min', 'ET', 'MODIS')
filepath = save_CSV(mn_df, 'MinET', 'monthly', village_name, 'MODIS')
print(f'completed: {filepath}')

hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'sum', 'SSEBop', 'b1')))
mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'mm/month', 'sum', 'b1', 'SSEBop')
filepath = save_CSV(mn_df, 'TotalET', 'monthly', village_name, 'SSEBop')
print(f'completed: {filepath}')

# hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'mean', 'SSEBop', 'b1')))
# mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'mm/month', 'mean', 'b1', 'SSEBop')
# filepath = save_CSV(mn_df, 'MeanET', 'monthly', village_name, 'SSEBop')
# print(f'completed: {filepath}')

# hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'median', 'SSEBop', 'b1')))
# mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'mm/month', 'median', 'b1', 'SSEBop')
# filepath = save_CSV(mn_df, 'MedianET', 'monthly', village_name, 'SSEBop')
# print(f'completed: {filepath}')

# hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'max', 'SSEBop', 'b1')))
# mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'mm/month', 'max', 'b1', 'SSEBop')
# filepath = save_CSV(mn_df, 'MaxET', 'monthly', village_name, 'SSEBop')
# print(f'completed: {filepath}')

# hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'min', 'SSEBop', 'b1')))
# mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'mm/month', 'min', 'b1', 'SSEBop')
# filepath = save_CSV(mn_df, 'MinET', 'monthly', village_name, 'SSEBop')
# print(f'completed: {filepath}')
