...
yr_df = get_rainfall(hyd_yr_col, '%Y', 'mm', 'sum')
filepath = save_CSV(yr_df, 'TotalRain', 'hydYear', village_name)
print(f'completed: {filepath}')

hyd_yr_col = ee.ImageCollection(ee.List(year_list).map(lambda year: yearly_sum(year, 'mean')))
yr_df = get_rainfall(hyd_yr_col, '%Y', 'mm/day', 'mean')
filepath = save_CSV(yr_df, 'MeanRain', 'hydYear', village_name)
print(f'completed: {filepath}')

hyd_yr_col = ee.ImageCollection(ee.List(year_list).map(lambda year: yearly_sum(year, 'median')))
yr_df = get_rainfall(hyd_yr_col, '%Y', 'mm/day', 'median')
filepath = save_CSV(yr_df, 'MedianRain', 'hydYear', village_name)
print(f'completed: {filepath}')

hyd_yr_col = ee.ImageCollection(ee.List(year_list).map(lambda year: yearly_sum(year, 'max')))
yr_df = get_rainfall(hyd_yr_col, '%Y', 'mm/day', 'max')
filepath = save_CSV(yr_df, 'MaxRain', 'hydYear', village_name)
print(f'completed: {filepath}')

# hyd_yr_col = ee.ImageCollection(ee.List(year_list).map(lambda year: yearly_sum(year, 'min')))
# yr_df = get_rainfall(hyd_yr_col, '%Y', 'mm/day', 'min')
# filepath = save_CSV(yr_df, 'MinRain', 'hydYear', village_name)
# print(f'completed: {filepath}')

hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'sum')))
mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'mm', 'sum')
filepath = save_CSV(mn_df, 'TotalRain', 'monthly', village_name)
print(f'completed: {filepath}')

hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'mean')))
mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'mm/day', 'mean')
filepath = save_CSV(mn_df, 'MeanRain', 'monthly', village_name)
print(f'completed: {filepath}')

hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'median')))
mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'mm/day', 'median')
filepath = save_CSV(mn_df, 'MedianRain', 'monthly', village_name)
print(f'completed: {filepath}')

hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'max')))
mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'mm/day', 'max')
filepath = save_CSV(mn_df, 'MaxRain', 'monthly', village_name)
print(f'completed: {filepath}')

# hyd_mn_col = ee.ImageCollection(ee.List(month_list).map(lambda yr_mn: monthly_sum(yr_mn, 'min')))
# mn_df = get_rainfall(hyd_mn_col, '%Y-%m', 'mm/day', 'min')
# filepath = save_CSV(mn_df, 'MinRain', 'monthly', village_name)
# print(f'completed: {filepath}')

da_df = get_rainfall(precipitation_collection, '%Y-%m-%d', 'mm', 'mean')
filepath = save_CSV(da_df, 'Rain', 'daily', village_name)
print(f'completed: {filepath}')
