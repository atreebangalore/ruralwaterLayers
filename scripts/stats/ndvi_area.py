...
unique_field = 'name'
data_to_use = 'Landsat7' # Landsat7, Landsat8, HLS or MODIS
start_year = 2009
end_year = 2023
mask_cropland = True
NDVI_THRESHOLD = 0.35
UPPER_NDVI_THRESHOLD = None # if not None
DW_CROP_LABEL = 4
...
roi = ee.FeatureCollection('users/balakumaranrm/karauli/karauli_taal_250_buffer')

dataFol = Path.home().joinpath("Data", "ag", "NDVI_area")
dataFol.mkdir(parents=True, exist_ok=True)
# print(f"layer to ee: {lyr_name}")

def feature_stats(image: ee.Image, fc: ee.FeatureCollection, threshold: float, scale_val: int, upper_threshold: float):
    class_image = image.gte(threshold).lte(upper_threshold) if upper_threshold else image.gte(threshold)
...
for yr in range(start_year,end_year+1):
    dict_key = f'ndvi_gte{NDVI_THRESHOLD}{f"_lte{UPPER_NDVI_THRESHOLD}" if UPPER_NDVI_THRESHOLD else ""}{"_DWcropMask" if mask_cropland else ""}_{data_to_use}_{yr}'
    out_dict[dict_key] = {}
    crop_mask = get_crop_mask(roi, '2021-06-01', '2022-03-01')#f'{max(yr, 2015)}-10-01', f'{max(yr, 2015)+1}-02-01')
    for mn in range(9,15):
        ...
        stats = feature_stats(ndvi, roi, NDVI_THRESHOLD, 30, UPPER_NDVI_THRESHOLD)
